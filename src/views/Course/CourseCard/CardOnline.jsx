import React from "react";
import { makeStyles } from "@material-ui/core/styles";
import Card from "@material-ui/core/Card";
import CardActionArea from "@material-ui/core/CardActionArea";
import CardContent from "@material-ui/core/CardContent";
import CardMedia from "@material-ui/core/CardMedia";
import Rating from "./Component/rating";
import PeopleIcon from '@material-ui/icons/People';
import Divider from '@material-ui/core/Divider';
import AlarmIcon from '@material-ui/icons/Alarm';
import Avatar from '@material-ui/core/Avatar';
import Online from "./img/online.png";
import Chip from '@material-ui/core/Chip';
import { Typography, Button } from "@material-ui/core";

const useStyles = makeStyles({
  root: {
    width: 290,
    borderRadius: '10px',
  },
  media: {
    height: 160,
  },
  tag: {
    height: 0,
    width: '110px',
  },
  top: {
    marginTop: '30px',
    backgroundImage: 'yellow',
  },
  white: {
    color: 'white',
  },
  detailbox: {
    backgroundColor: '#4D5359',
    padding: '10px',
    marginTop: '25px',
    textAlign: 'left',
    width: '100%',
    color: 'white',
    // 'box-shadow': '20px 20px #BCD8B7',
    borderTopLeftRadius: '10px',
    borderTopRightRadius: '10px',
  },
  authorbox: {
    backgroundColor: '#3C7A89',
    padding: '3px',
    width: '100%',
    color: 'white',
    // 'box-shadow': '20px 20px #BCD8B7',
    borderBottomRightRadius: '10px',
    borderBottomLeftRadius: '10px',
  }
});

export default function MediaCard(props) {
  const classes = useStyles();

  function Hot() {
    if (props.hot === "true") {
      return <img alt="" width="24px" height="24px" style={{ display: 'inline' }} src="data:image/svg+xml;base64," />
    }
  }

  function New() {
    if (props.new === "true") {
      return <img alt="" width="40px" height="40px" style={{ display: 'inline' }} src="data:image/svg+xml;base64," />
    }
  }

  return (

    <div className={classes.top}>
      <div className={classes.tag}>
        <img alt="Online" src={Online} width="100" height="100" />
      </div>
      <Card className={classes.root}>
        <CardActionArea>
          <CardMedia
            className={classes.media}
            image={props.img}
            title="Contemplative Reptile"
          />
          <CardContent style={{ minHeight: '293px' }}>
            <div>
              <Typography variant="h5" component="h2" style={{ textAlign: 'left', fontSize: '20px', marginBottom: 5, color: 'black', fontWeight: '700', height: 80 }}>
                {props.coursename}  {Hot()} {New()} {props.id}
              </Typography>
            </div>
            <div style={{ textAlign: "left", marginBottom: 5 }}>
              <Button size="small" style={{ color: "#00000080", minWidth: 0, padding: 0, marginRight: 5 }}>#{props.category1} </Button>
              <Button size="small" style={{ color: "#00000080", minWidth: 0, padding: 0, marginRight: 5 }}>#{props.category2} </Button>
            </div>
            <div style={{ display: "flex" }}>
              <Rating rating={props.rating}></Rating>
              <Typography variant="subtitle1" style={{ fontSize: 17, marginLeft: 10 }}>{props.rating}{" คะแนน"}</Typography>
            </div>

            <div style={{ display: "flex", alignItems: "center", marginTop: 8 }}>
              <Avatar
                style={{ backgroundColor: 'orange' }}
                width='40px'
                height='40px'
                src={`${process.env.PUBLIC_URL}/assets/img/p1.svg`} />
              <Typography variant="h6" style={{ fontWeight: 400, marginLeft: 5, textAlign: 'left', 'text-overflow': 'ellipsis', overflow: 'hidden', 'white-space': 'nowrap' }}>{" "}{props.teacher}{" "}</Typography>
            </div>

            <Divider style={{ marginTop: 40, marginBottom: 10 }} />
            <div style={{ display: "flex", alignItems: "center", color: "#000000c4" }} >
              <PeopleIcon style={{ marginRight: 5 }} /> {``}
              <Typography variant="body2" style={{ marginLeft: 4 }}>{" "}<span style={{ fontWeight: 500 }}>{props.people}</span>{" คน"}</Typography>
            </div>
            <div style={{ display: "flex", alignItems: "center", color: "#000000c4" }} >
              <AlarmIcon style={{ marginRight: 5 }} /> {``}
              <Typography variant="body2" style={{ marginLeft: 4 }}>{" "}<span style={{ fontWeight: 500 }}>{props.time}</span>{" hours"}</Typography>
            </div>
          </CardContent>
        </CardActionArea>

      </Card>
    </div>

  );
}
